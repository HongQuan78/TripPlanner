...
- **Endpoints:** `POST /api/auth/register` → 200 `{message}` (generic for duplicates too); `POST /api/auth/login` → 200 `{id, email, role, token}` / 401 ProblemDetails with generic `Invalid email or password.` (also when the account is unverified — do not invent a "verify first" message); `POST /api/auth/logout` (Bearer) → 200; `GET /api/auth/verify-email?token=` → 200 `{message}` / 400; `POST /api/auth/resend-verification` `{email}` → always 200 `{message}` (60s server-side cooldown returns the same generic success — no special UI needed).
