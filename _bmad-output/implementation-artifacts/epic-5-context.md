...
- **Anti-enumeration is non-negotiable.** No single UI interaction may reveal whether an email has an account. Register success copy is identical for fresh and duplicate emails; a login rejected for a wrong password or an unknown email shows the generic invalid-credentials message; resend-verification shows the same generic success during its server-side cooldown, with no countdown implying knowledge of server state. **Narrowed 2026-07-26 (story `4-5-unverified-login-message`):** the *unverified* state is disclosed once the password verifies — that login returns `Your email address is not verified. Please check your inbox.` Render backend messages verbatim and do not collapse them back to one voice.
