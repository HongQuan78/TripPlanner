...
`Restaurant` is deliberately a **pure category marker** — it overrides `Category` and adds no fields of its own. It used to carry `CuisineType` and `IsHalalFriendly`, both predating the OpenTripMap integration; OpenTripMap supplies neither a cuisine nor a halal signal, so every xid-imported restaurant persisted the raw provider *kind code* (`"foods"`) as its cuisine with `IsHalalFriendly` hardcoded `false`. Both were dropped (migration `RemoveRestaurantCuisineFields`), along with the two `DestinationResponse` fields and the frontend's `Destination` type members, which nothing rendered. Do not re-add them unless a provider that actually supplies the data is integrated first. `Landmark.OpeningHours` is *not* in the same category — it is genuinely sourced from Overpass. The subclass still earns its keep: `DestinationCategoryHelper.IsRestaurantCategory` chooses it over `Landmark` at import time, and it drives `DestinationResponse.Category`.
