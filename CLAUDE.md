...
**External services:** Application defines service interfaces (`IGeocodingService`, `IAttractionSearchService`, `IDestinationDetailsService` in `Application/Interfaces/Services/`); Infrastructure implements them against the OpenTripMap API in `Infrastructure/ExternalServices/OpenTripMap/`. Clients are registered via `AddHttpClient<TInterface, TImplementation>` with `OpenTripMapSettings` (bound from configuration) providing base URL, API key, and timeout. External API failures surface as `ErrorType.ServiceUnavailable` results, not exceptions. Follow this pattern for any new third-party API integration. `IWikipediaImageService` (implemented by `WikipediaImageService` in `Infrastructure/ExternalServices/Wikipedia/`) provides a free image fallback via Wikipedia's public REST summary API — no API key involved, unlike the OpenTripMap/Resend integrations — consumed by both `OpenTripMapAttractionSearchService` and `OpenTripMapDestinationDetailsService` when OpenTripMap's own `preview`/`image` fields are absent.
