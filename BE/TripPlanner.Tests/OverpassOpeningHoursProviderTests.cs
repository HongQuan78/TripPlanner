...
public class OverpassOpeningHoursProviderTests
{
    ...
    public async Task GetOpeningHoursAsync_NodeXidWithOpeningHours_ReturnsHoursAndQueriesNode()
    {
        ...
        Assert.Equal("Mo-Fr 09:00-17:00", result.Value);
        Assert.Equal("[out:json][timeout:5];node(191031796);out tags;", handler.LastDecodedBody);
    }
    ...
    public async Task GetOpeningHoursAsync_WayXid_QueriesWay()
    {
        ...
        Assert.Equal("24/7", result.Value);
        Assert.Equal("[out:json][timeout:5];way(123);out tags;", handler.LastDecodedBody);
    }
    ...
    public async Task GetOpeningHoursAsync_RelationXid_QueriesRel()
    {
        ...
        Assert.Equal("Mo-Su 08:00-20:00", result.Value);
        Assert.Equal("[out:json][timeout:5];rel(456);out tags;", handler.LastDecodedBody);
    }
    ...
    public async Task GetOpeningHoursAsync_EmptyElements_ReturnsNull()
    {
        ...
        Assert.Equal(OpeningHoursAvailability.KnownAbsent, result.Availability);
    }
    ...
    public async Task GetOpeningHoursAsync_HttpRequestException_ReturnsNull()
    {
        ...
        Assert.Null(result.Value);
        Assert.Equal(OpeningHoursAvailability.Unavailable, result.Availability);
    }
    ...
    public async Task GetOpeningHoursAsync_SecondCall_ServedFromCacheWithoutHttpCall()
    {
        ...
        Assert.Equal("Mo-Fr 09:00-17:00", first.Value);
        Assert.Equal("Mo-Fr 09:00-17:00", second.Value);
    ...
    [Fact]
    public async Task GetOpeningHoursAsync_RuntimeErrorRemarkWithHttp200_IsUnavailableNotAbsent()
    {
        var handler = JsonHandler("""{"version":0.6,"elements":[],"remark":"runtime error: Query timed out in \"query\" at line 1 after 26 seconds."}""");
        var (provider, _) = CreateProvider(handler);
    ...
    [Fact]
    public async Task GetOpeningHoursAsync_RuntimeErrorRemarkWithHttp200_CachedWithShortTtlOnly()
    {
        var handler = JsonHandler("""{"elements":[],"remark":"runtime error: Query run out of memory."}""");
        var (provider, cache) = CreateProviderWithRecordingCache(handler);
    ...
    [Fact]
    public async Task GetOpeningHoursAsync_CachedFailure_StaysUnavailableOnSecondCall()
    {
        var handler = new FakeHttpMessageHandler(_ => new HttpResponseMessage(HttpStatusCode.TooManyRequests));
        var (provider, _) = CreateProvider(handler);
        ...
        Assert.Equal(OpeningHoursAvailability.Unavailable, second.Availability);
        Assert.Equal(1, handler.CallCount);
    }

    [Fact]
    public async Task GetOpeningHoursAsync_ConfiguredCacheMinutes_IsUsedForDefinitiveAnswer()
    {
        var handler = JsonHandler("""{"elements":[{"tags":{"opening_hours":"24/7"}}]}""");
        var httpClient = new HttpClient(handler)
        {
        ...
        var cache = new RecordingResponseCache();
        var provider = new OverpassOpeningHoursProvider(
            httpClient,
            Options.Create(new OverpassSettings { CacheMinutes = 30 }),
            cache);

        await provider.GetOpeningHoursAsync(new OpeningHoursContext { Xid = "N1" });

        var write = Assert.Single(cache.Writes);
        Assert.Equal(TimeSpan.FromMinutes(30), write.Ttl);
    }

    [Fact]
    public async Task GetOpeningHoursAsync_ConfiguredTimeout_IsSentAsServerSideQueryTimeout()
    {
        var handler = JsonHandler("""{"elements":[{"tags":{"opening_hours":"24/7"}}]}""");
        var httpClient = new HttpClient(handler)
        {
        ...
        var provider = new OverpassOpeningHoursProvider(
            httpClient,
            Options.Create(new OverpassSettings { TimeoutMilliseconds = 9000 }),
            TestCache.Create());

        await provider.GetOpeningHoursAsync(new OpeningHoursContext { Xid = "N1" });

        Assert.Equal("[out:json][timeout:9];node(1);out tags;", handler.LastDecodedBody);
    }

    [Fact]
    public async Task GetOpeningHoursAsync_NonAsciiDigitsInXid_RejectedWithoutHttpCall()
    {
        var handler = JsonHandler("""{"elements":[{"tags":{"opening_hours":"24/7"}}]}""");
        var (provider, capturedHandler) = CreateProvider(handler);

        var result = await provider.GetOpeningHoursAsync(new OpeningHoursContext { Xid = "N١٢٣" });
    ...
    public async Task GetOpeningHoursAsync_CaseVariantXid_SharesOneCacheEntry()
    {
        ...
        Assert.Equal("24/7", upper.Value);
        Assert.Equal("24/7", lower.Value);
        Assert.Equal(1, handler.CallCount);
    }
}
