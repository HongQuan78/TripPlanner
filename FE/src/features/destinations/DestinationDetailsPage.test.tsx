...
const fullDetails: DestinationDetails = {
  ...
  openingHoursAvailability: 'Available',
...
describe('DestinationDetailsPage', () => {
  ...
  it('shows a Closed badge when the hours parse and it is currently shut', async () => {
  ...
  it('uses the exact absent-hours literal when the provider knows there are none', async () => {
    getDestinationDetailsMock.mockResolvedValue({
      ...fullDetails,
      openingHours: null,
      openingHoursAvailability: 'KnownAbsent',
  ...
  it('distinguishes a failed hours lookup from a definitive absence', async () => {
    getDestinationDetailsMock.mockResolvedValue({
      ...fullDetails,
      openingHours: null,
      openingHoursAvailability: 'Unavailable',
    });
    renderPage();

    await waitFor(() => {
      expect(screen.getByText('Opening hours could not be loaded')).toBeInTheDocument();
    });
    expect(screen.queryByText('Opening hours not available')).not.toBeInTheDocument();
  });
